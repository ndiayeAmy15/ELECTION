...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
	...
	<script>
	  ...
	  document.addEventListener('DOMContentLoaded', function () {
		  ...
		  fetch('/graph')
			  ...
			  .then(data => {
				  const candidats = data.chartData.map(item => item.nom + ' ' + item.prenom);
				  const votes = data.chartData.map(item => item.likes);
	...
	{{-- <script>
        ...
        var likeData = {!! json_encode($likeData) !!}; // Les données de votre modèle

        // Logique pour extraire les données nécessaires de votre modèle et les utiliser dans le graphique

        var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: [likeData->prenom], // Labels pour l'axe X
                datasets: [{
                    label: 'Votre graphique',
                    data: [likeData->nom], // Données pour l'axe Y
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    borderColor: 'rgba(75, 192, 192, 1)',
                    borderWidth: 1
                }]
...
{{-- 
	<script>
        ...
        var ctx = document.getElementById('likegraph').getContext('2d');
