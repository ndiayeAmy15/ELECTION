$('change_pwd').click(function () {
    ...
    var cpwd = $('#confirm_new_password').val();
    if (pwd != && /^[a-zA-Z0-9]{6,9}$/.test(pwd) && pwd.length >= 8) {
        $('#pwd').removeClass('is-invalid');
        $('#pwd').addClass('is-valid');
        $('#error-register-pwd').text("");
    } else {
        $('#pwd').addClass('is-invalid');
        $('#pwd').removeClass('is-valid');
        $('#error-register-pwd').text("password is invalid");
    }
    if (cpwd != "" && cpwd == pwd && /^[a-zA-Z0-9]{6,9}$/.test(cpwd) && pwd.length >= 8) {
        $('#cpwd').removeClass('is-invalid');
        $('#cpwd').addClass('is-valid');
        $('#error-register-cpwd').text("");
    }
});
